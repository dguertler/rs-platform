...
# Ab diesem realisierten GEWINN bekommt ein Verkauf der Woche einen eigenen
# Kauf-/Verkauf-Chart. Mehrere Gewinner >= Schwelle ergeben mehrere Slides.
SELL_SLIDE_THRESHOLD = 0.10
...
def _big_sells(universe, closed_raw, kw, as_of=None):
    """ALLE realisierten Trades der laufenden KW mit Gewinn >= Schwelle und genug
    Chart-Metadaten (Kauf/Verkauf). Sortiert nach Rendite absteigend → je ein
    eigener Kauf-/Verkauf-Chart. Leere Liste, wenn keiner qualifiziert."""
    out = []
    for t in closed_raw:
        if t.get("kw") != kw or t.get("ticker") not in universe:
            continue
        if not t.get("buy_date") or not universe[t["ticker"]]["ohlcv"]:
            continue
        if t.get("ret", 0) < SELL_SLIDE_THRESHOLD:
            continue
...
def compute(kw, universe, benchmark, ref_date=None):
    ...
    # ── „Große Verkäufe der Woche" ───────────────────────────────────────────
    # Alle in dieser KW realisierten Gewinner (>= Schwelle) bekommen je einen
    # Kauf-/Verkauf-Chart; diese ersetzen die „Weitere Positionen"-Slide und der
    # Newcomer entfaellt (siehe generate.build_from_store).
    big_sells = _big_sells(universe, closed_raw, kw, as_of)
